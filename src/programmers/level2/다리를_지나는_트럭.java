...
public class 다리를_지나는_트럭 {
    public int solution(int bridge_length, int weight, int[] truck_weights) {
        ...
        for (int truck_weight : truck_weights) {
            truckList.add(truck_weight);
        }
        // 초기 트럭 하나 올리기
        int realTimeWeight = 0;
//        bridge.add(truck_weights[0]);
        int lateTime = bridge_length;

        do  {

            // 트럭 올라타기 (트럭이 남아있을때)
            if (!truckList.isEmpty()) {
                if (realTimeWeight + truckList.peek() <= weight && bridge.size() < bridge_length) { // 무게 초과 안될때
                    realTimeWeight += truckList.peek();
                    bridge.add(truckList.poll());
